...
export default function DocumentUpload({ carrierId }: { carrierId: string }) {
  const router = useRouter();
  const fileRef = useRef<HTMLInputElement>(null);
  const [type, setType] = useState<DocType>("coi");
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    const { error: insErr } = await supabase
      .from("documents")
      .insert({ carrier_id: carrierId, type, file_path: path, status: "pending" });
    if (insErr) {
  ...
  return (
    ...
    >
      <h2 className="text-sm font-semibold text-textPrimary">Upload a document</h2>
